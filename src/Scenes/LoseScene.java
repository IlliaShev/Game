...
public class LoseScene extends Scene {
    ...
    private String loseImageURL = "resources\\images\\Lose.gif";
    private LevelScene levelScene;
    private Button repeatButton;
    private Button backButton;
    private Timeline timeline;

    public LoseScene(GridPane pane, int FRAME_WIDTH, int FRAME_HEIGHT, LevelScene levelScene) throws IOException {

        super(pane, FRAME_WIDTH, FRAME_HEIGHT);
        this.levelScene = levelScene;

        loseLabel = new Label("Ви програли битву, та не війну!");
        loseLabel.setFont(Font.font("Candara", FontWeight.BOLD, 50));
        loseLabel.setTextFill(Paint.valueOf("WHITE"));

        InputStream stream = new FileInputStream(loseImageURL);
        ...
        loseMenu = new VBox(loseLabel,imageView,repeatButton,backButton);
        loseMenu.setAlignment(Pos.CENTER);
        loseMenu.setSpacing(15);
        ...
        GridPane loseGroup = new GridPane();
        loseGroup.getChildren().add(loseMenu);
        loseGroup.setAlignment(Pos.CENTER);

        pane.setPrefWidth(FRAME_WIDTH);
        pane.setPrefHeight(FRAME_HEIGHT);
        pane.getChildren().add(background);
        pane.getChildren().add(loseGroup);
...
//                (Paint.valueOf("DARKCYAN"), CornerRadii.EMPTY, Insets.EMPTY)));
    ...
    private void playBackMusic() {
        try {
            File soundFile = new File("resources/music/Lose.wav");
